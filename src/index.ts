...
async function getResponse(request: RequestWithKey, env: Env, context: ExecutionContext) {
    ...
    try {
        ...
        let userAgent = getUserAgentGroups(request.headers.get("X-User-Agent") ?? request.headers.get("User-Agent"));
        ...
        const response = await getRouterRequest(request, env, context, databaseSource, executionFeatureFlags);
...
async function getRouterRequest(request: Request, env: Env, context: ExecutionContext, databaseSource: DatabaseSource, featureFlags: FeatureFlagsExecution) {
...
export default {
    async fetch(request: RequestWithKey, env: Env, context: ExecutionContext) {
        const response = await getResponse(request, env, context);

        response.headers.set("Access-Control-Allow-Origin", "*");
        response.headers.set("Access-Control-Allow-Methods", "GET, POST, PUT, PATCH, DELETE, OPTIONS");
        response.headers.set("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept, Authorization, X-User-Agent");
