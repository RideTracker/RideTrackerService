...
export async function getActivityCommentsSummary(database: D1Database, activity: string): Promise<(ActivityComment & { comments_count?: number })[] | null> {
    return (await database.prepare(
        ...
        ).bind(activity).all<ActivityComment & { comments_count?: number }>()).results ?? null;
};
